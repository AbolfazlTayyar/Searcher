...
2. **~81% of rows (271/336) are malformed** — unescaped quotes in free-text fields (e.g. `summary`) shift columns for the rest of that row. ~16% (53/336) shift the row's total field count and are caught by a header-length check; the remaining ~65% (218/336) shift by an amount that nets out to the *same* field count (one field's quoting absorbs a comma that should've started a new column, another field downstream splits in two), so they also need per-field shape validation on top of the length check — e.g. `industry`/`job_title`/`summary` never legitimately hold a Python-literal repr or a bare phone number, `summary` is never a bare int/float, `skills` (a syntactically valid list-of-strings either way) is never a list of phone numbers, and `linkedin_url`/`facebook_url`/`twitter_url`/`github_url`/`gender` have a well-known shape wherever they land. The source data also contains exact duplicate rows (~10%, 34/336) that must be deduped rather than double-indexed. The ingestion script MUST validate each row's field count against the header, its field shapes against those anchor checks, and its exact-duplicate status before trusting it — skip + log malformed/duplicate rows rather than indexing garbage. Log skipped rows (index + reason) to `backend/ingest.log`. Only ~9% of rows (31/336) survive all three checks clean.
