...
2. **حدود ۸۱٪ از سطرها (۲۷۱ از ۳۳۶) نامعتبرند.** کوتیشن‌های بدون escape در فیلدهای متنی آزاد (مثل `summary`) ستون‌های باقی‌ماندهٔ سطر را جابه‌جا می‌کنند. ۵۳ سطر (~۱۶٪) تعداد کل فیلد را هم عوض می‌کنند و صرفاً با مقایسهٔ طول سطر با هدر شناسایی می‌شوند. ۲۱۸ سطر دیگر (~۶۵٪) جابه‌جایی‌ای دارند که تعداد فیلد را ثابت نگه می‌دارد (یک فیلد کاما را می‌بلعد، فیلدی دیگر جلوتر دو تکه می‌شود)، پس `parse_profiles` شکل تک‌تک فیلد را هم می‌سنجد — `industry`/`job_title`/`summary` نباید لیترال پایتون یا شمارهٔ تلفن خام باشند، `summary` نباید عدد خام هم باشد، `skills` (که به‌عنوان لیستی از رشته‌ها همچنان معتبر است) نباید لیستی از شماره‌تلفن باشد، فیلدهای URL پلتفرم‌ها (`linkedin_url` و مشابه) باید دامنهٔ خودشان را داشته باشند — و در صورت مغایرت کل سطر رد می‌شود، نه فقط آن فیلد. ۳۴ سطر (~۱۰٪) هم عیناً تکراری‌اند و حذف می‌شوند تا دوبار ایندکس نشوند. از ۳۳۶ سطر در نهایت فقط ۳۱ تا (~۹٪) سالم می‌مانند؛ همهٔ ردشدن‌ها با شماره‌سطر و دلیل در `backend/ingest.log` ثبت می‌شوند.
